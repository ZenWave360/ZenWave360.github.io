...
Inspired by JHipster JDL, and largely compatible with its entities and relationships format.

ZDL is a language for describing domain entities and their relationships, as well as services, commands, events and business policies. Reference external API definitions, and configure SDK plugins.

It's designed to be compact, while still being readable and expressive. It's also designed to be business friendly, developer friendly, and machine friendly.


It works as an Ubiquitous Language
...
- A prolog section with `global javadoc`, `config` and `apis` sections that are used by ZenWave IntelliJ Editor and ZenWave SDK plugins for code generation.
- A DDD and EDA sections with `entities`, `enums`, `services`, `inputs`, `outputs` and `events` declarations.
...
The following is an example of a ZDL definition file describing an application with an aggregate exposing some core functionality, connected through a REST API adapter and publishing some domain events.
...
- `String`, `Integer`, `Long`, `BigDecimal`, `Float`, `Double`, `Enum`, `Boolean`, `LocalDate`, `ZonedDateTime`, `Instant`, `Duration`, `UUID`, `Blob`, `AnyBlob`, `ImageBlob`, `TextBlob`.
...
relationship (OneToMany | ManyToOne | OneToOne | ManyToMany) {
  ...
  <from entity>[{<relationship field name>}] to
  [<to relationship javadoc>]
  [<to relationship annotation>*] <to entity>[{<relationship field name>}]
}
```
...
relationship ManyToMany {
  ...
  EntityC{fieldPointingD} to EntityD{fieldPointingToC}
}
```
...
`CommandOutput` can be any valid `entity` or `output` type, of be unspecified. If CommandOutput is an entity, it expresses that entity will be created or updated with the command input data. CommandOutput can be marked as `optional` with a `?` to express that the command may not return any output.
