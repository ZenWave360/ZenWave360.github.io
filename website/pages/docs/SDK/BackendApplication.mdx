...
With ZenWave SDK you can quickly convert your ZDL models, the results of your analysis and design, into a Spring Boot/Spring Cloud backend application.
...
Depending on your particular use case you have different ways to start modeling with ZDL:

- **Entities and Aggregates:** Traditional you would start your application modeling your data.
...
Start describing your data model with the `entity` keyword.

You can describe `fields` with `fieldName` `fieldType`, `validations` and `javadoc comments`.

These javadoc comments can either precede the field definition on a separate line or be appended to the end of the line where the field is defined, which is a departure from the typical Java convention.
...
Fields can also be arrays of the above types or references, but direct references and arrays only work with Mongodb, for JPA you will need to configure a `OneToOne`, `ManyToOne`, `ManyToOne`, `ManyToMany`... relationship, and direct references will work only in places where JPA `@Embeddable` is allowed.
...
ZDL Modeling Language allows any custom annotation on entities and fields, it's the job of ZenWave Plugins to interpret them and generate code accordingly.
...
Sure, here is the reordered table with "Persistence" and "Repository" as the last columns:
...
The most important ones are `@aggregate`, `@embedded`, `@copy` and `@extends`.
...
`@embedded` entities do not contain id, createdDate, lastModifiedDate... and will not have a Spring Data Repository interface generated for them.

You can reuse entity definitions or create a class hierarchy using `@copy` and `@extends` annotations.
...
While `@extend` will create a class hierarchy, `@copy` will copy all the fields from the referenced entity into the current entity and can be used when you need to reuse fields in entities that belong to different packages (like domain, inputs, outpus and events)
...
In the context of Domain-Driven Design (DDD), is the combination of entities annotated with `@aggregate` and the associated service commands collectively what form an Aggregate. Encapsulating a cluster of domain objects that can be treated as a single unit, ensuring consistency and enforcing business rules.

This combination is sometimes referred as [Anemic Domain Model](https://en.wikipedia.org/wiki/Anemic_domain_model). We call it **Separation of Concerns**, clearly separating logic from data and works just fine for most microservice applications.

Services are still the only entry point for your aggregates and it greatly simplifies how domain events are published to external brokers.
...
service CustomerService for (Customer) {
  // this will produce the following Java method signature:
  // Page<Customer> searchCustomers(String firstName, String lastName, String email)
  @paginated
  searchCustomers(CustomerFilter) Customer[]
}
...
You can also create specific `input` and `output` types for service commands parameters and return types. But you can directly use any `entity` type.

There is the option `inputDTOSuffix` in ZenWave SDK Backend Plugin to automatically generate an input class for your entities when is not empty.

If you use an `entity` as command parameter, and you don't set `inputDTOSuffix` ZenWave SDK will still use a mapstruct mapper to copy parameter fields into persistent entity.
...
As always, you can run ZenWave SDK plugin from the command line using jbang:
