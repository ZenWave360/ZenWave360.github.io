...
# Modeling Aggregates
Following DDD Aggregates principles.

DDD Aggregates principles:
...
ZenWave SDK supports these three common patterns to model aggregates:

- [Lightweight/Data Centric:](#lightweight-aggregates) When your application logic is simple, keep your implementation simple.
- [Rich Domain Aggregates:](#rich-domain-aggregates) When you are dealing with complex business logic and core subdomains.
...
Most microservice applications look like: <strong className="orange">"Receive some command/request, validate, process, persist the data and emit an integration event for downstream services."</strong>
...
Use the <strong className="orange">@aggregate</strong> annotation to mark your <strong className="orange">aggregate root entity</strong>, and use **service** for defining command/methods:
...
With ZenWave SDK/ZDL you can model aggregates that are event sourced. This is a powerful pattern that allows you to store the state of an aggregate as a sequence of events and then reconstruct the aggregate state by replaying the events.
...
This approach is just an extension of the Rich Domain Aggregates pattern, where aggregate commands are event handlers that reconstruct the aggregate state, and can be used also for modeling your domain with the CQRS pattern.
